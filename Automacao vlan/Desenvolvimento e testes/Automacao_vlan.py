...
for Vlan_ID, Vlan_Name in Vlans_padrao.items():
   ...
   if Command_vlan == 'VLAN id ' + str(Vlan_ID) +' not found in current VLAN database':
      ...
      Vlan_criada = [item['vlan_name'] for item in Command_vlan if item['vlan_id'] == str(Vlan_ID)]
      if [item['vlan_id'] for item in Command_vlan if item['vlan_name'] == str(Vlan_Name)]:
         print('Vlan já estava criada e com o ID e nome correto')
      else:
