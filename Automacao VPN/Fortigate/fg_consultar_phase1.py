...
if response.status_code == 200:
    ...
    print("\nConsulta de VPN IPSec Phase 1: OK")

    vpns = dados.get("results", [])

    if len(vpns) == 0:
        print("\nNenhuma Phase 1 configurada.")
    else:
        print("\nPhase 1 encontradas:")

        for vpn in vpns:
            print(f"\nNome: {vpn.get('name')}")
            print(f"Interface: {vpn.get('interface')}")
            print(f"Remote Gateway: {vpn.get('remote-gw')}")
            print(f"IKE Version: {vpn.get('ike-version')}")
            print(f"Proposal: {vpn.get('proposal')}")
            print(f"DH Group: {vpn.get('dhgrp')}")

else:
    print("\nErro ao consultar as configurações de Phase 1:")
    print(response.text)